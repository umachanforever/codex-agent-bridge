...
The [pinned Responses client](https://github.com/openai/codex/blob/rust-v0.155.1/codex-rs/core/src/client.rs) still puts tool schemas into prompt-only `AdditionalTools` items and disables parallel tool calls when `use_responses_lite` is true, so the catalog workaround remains necessary. The [model cache](https://github.com/openai/codex/blob/rust-v0.155.1/codex-rs/models-manager/src/cache.rs) retains versioned cache metadata; the proxy preserves it in its separate catalog, which Codex loads through `model_catalog_json`. This does not refresh the frozen catalog or prove live tool compatibility.
